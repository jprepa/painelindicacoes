...
import { Phone, MapPin, Briefcase, TrendingUp, Filter, Calendar, User, Clock, ArrowRight, Plus, History, CheckCircle } from 'lucide-react';
...
const AbaParceiros = () => {
  ...
  const aoAdicionarParceiro = (novoParceiro) => {
    setParceiros([novoParceiro, ...parceiros]);
    setParceiroSelecionado(novoParceiro);
  };
  ...
  const registrarMovimentacao = async (tipo) => {
    ...
    // Define valor padrão baseado no botão clicado
    ...
    if (pontos) {
      try {
        const response = await axios.post(`https://prevision-backend.onrender.com/api/parceiros/${parceiroSelecionado.id}/registrar_indicacao/`, {
          pontos: pontos,
          tipo: tipo // Envia se é Venda ou Indicação
        });
        ...
        alert("Erro no servidor. Verifique se o backend foi atualizado.");
      }
    }
  };

  // --- LÓGICA DE FILTRAGEM ---
  // Extrai lista única de estados e serviços para preencher os Selects
        ...
        <div className="p-4 border-b border-gray-100 bg-white space-y-3">
          ...
          <div className="space-y-2">
             <input 
                ...
                className="w-full text-sm p-2 border rounded-lg bg-gray-50"
                value={filtroTexto}
                onChange={e => setFiltroTexto(e.target.value)}
             />
             <div className="flex gap-2">
                <select 
                    className="w-1/2 text-xs p-2 border rounded-lg bg-white"
          ...
          <button 
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg flex items-center justify-center gap-2 font-medium text-sm transition shadow-sm mt-2"
          >
          ...
          {loading ? (
            <p className="text-center text-gray-400 mt-10">Carregando...</p>
              ...
              >
                ...
                {/* MOSTRA ESTADOS NA LISTA TAMBÉM */}
                <div className="flex items-center gap-1 mt-1">
                    {p.estados_lista && p.estados_lista.slice(0, 3).map(uf => (
                        <span key={uf} className="text-[10px] bg-gray-100 text-gray-600 px-1 rounded border border-gray-200">{uf}</span>
                    ))}
                    {p.estados_lista && p.estados_lista.length > 3 && <span className="text-[10px] text-gray-400">...</span>}
                </div>
              </div>
            )
          )}
          {parceirosFiltrados.length === 0 && !loading && (
             <p className="text-center text-xs text-gray-400 mt-4">Nenhum parceiro encontrado.</p>
          )}
        </div>
      </div>
      ...
      <div className="w-2/3 overflow-y-auto bg-gray-50 p-6 md:p-8">
        {parceiroSelecionado ? (
          <div className="max-w-4xl mx-auto space-y-6 pb-20">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
              <div className="flex justify-between items-start relative z-10">
                ...
                <div className="flex flex-col items-end gap-3">
                  ...
                  <div className="flex gap-2">
                      <button 
                        onClick={() => registrarMovimentacao('Indicação')}
                        className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-2 px-3 rounded-lg shadow-sm transition-all active:scale-95"
                      >
                        + Indicação
                      </button>
                      <button 
                        onClick={() => registrarMovimentacao('Venda')}
                        className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-2 px-3 rounded-lg shadow-sm transition-all active:scale-95"
                      >
                        $$ Venda
                      </button>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
               ...
               {/* BLCO DE PREVISÃO SIMPLIFICADO AQUI (Pode manter o seu original se preferir) */}
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                 <div>
                    <h3 className="font-bold text-gray-700 flex items-center gap-2 mb-1">
                        <Clock size={18} className="text-orange-500"/> Inatividade
                    </h3>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
                    <History size={18} className="text-purple-500"/> Histórico de Pontuação
                </h3>
                                    ...
                                    <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded text-xs font-bold ${
                                                item.tipo === 'Venda' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                            }`}>
                                                {item.tipo}
                                            </span>
                                        </td>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {parceiroSelecionado.servicos_lista && parceiroSelecionado.servicos_lista.length > 0 ? (
                    parceiroSelecionado.servicos_lista.map((servico) => (
                    <div key={servico} className="px-3 py-2 rounded-lg text-xs font-semibold border bg-gray-50 text-gray-600 border-gray-200">
                        {servico}
                    </div>
                    ))
          ...
          <div className="h-full flex flex-col items-center justify-center text-gray-400 opacity-50">
             ...
             <p>Selecione um parceiro ao lado para ver os detalhes</p>
          </div>
        )}
      </div>
    </div>
